...
class Daemon(object):
    ...
    """
    systemInfo = {}
    path = "outputJsonFiles"
    syslogPath = "outputJsonFiles/syslogs"
    updatedSyslogPath = "outputJsonFiles/updatedSyslogs"
    logFilePath = [
        ...
        # "/var/log/ufw.log",
        # "/home/dev/Documents/somewhere/custom.log",
        "/home/dev/Documents/something.log"
    ]
    def __init__(self, stdin='/dev/null', stdout='/dev/null', stderr='/dev/null'):
        ...
        self.stdout = stdout
        self.stderr = stderr
        self.__UpdateCustomLogs()
        print(self.logFilePath)
    
    # MrX7 (AAM) Work
    # Ability to add more custom logs to read
    def __UpdateCustomLogs(self):
        with open('/etc/opt/redsq/agent_config/syslog.txt', 'r') as cLogs:
            for line in cLogs.readlines():
                self.logFilePath.append(line)
    ...
    def get_linux_password_info(self, username):
        try:
            passwd_result = subprocess.run(['passwd', '-S', username], capture_output=True, text=True, check=True)
            chage_result = subprocess.run(['sudo', 'chage', '-l', username], capture_output=True, text=True, check=True)
            password_settings = self.parse_chage_output(chage_result.stdout)
            formatted_settings = "\n".join(f"{key}: {value}" for key, value in password_settings.items())
            return f"Password Info:\n{passwd_result.stdout}\n\nChage Info:\n{formatted_settings}"
        except subprocess.CalledProcessError as e:
            return f"Error retrieving password info: {e}"

    def get_linux_password_settings(self, username):
        try:
            result = subprocess.run(['sudo', 'chage', '-l', username], capture_output=True, text=True, check=True)
            return self.parse_chage_output(result.stdout)
        except subprocess.CalledProcessError as e:
            return f"Error retrieving password settings: {e}"
        
    def get_linux_hard_drive_serial_numbers(self):
        try:
            result = subprocess.run(['sudo', 'lsblk', '-o', 'name,serial'], capture_output=True, text=True, check=True)
            return result.stdout
        except subprocess.CalledProcessError as e:
            return f"Error detecting hard drives: {e}"
        

    def parse_chage_output(self, chage_output):
        settings = {}
        lines = chage_output.splitlines()
        for line in lines:
